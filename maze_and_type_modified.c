...
int main()
{
    ...
    for (int witch_people = 0; witch_people < num_people; witch_people++)
    {
        ...
        for (int i = 0; i < num_dish; i++)
        {
            ...
            if (hard)
                limit_time += now_dish->num_ingredient;
        ...
        while (1)
        {
            ...
            printf_Maze(maze);
            time(&endTime);
            if (difftime(endTime, startTime) >= TIME)
            { // 若遊玩時間大於30秒，遊戲停止
                printf("Time is up!");
...
void convert_letters(struct node_ty *first, int num_dish)
{
    ...
    for (int i = 0; i < num_dish; i++)
    {
        ...
        for (int j = 0; j < len; j++)
        {
            ...
            {
                if (x)
                    temp_dish->ingredient.word[j] = toupper(temp_dish->ingredient.word[j]);
                else
                    temp_dish->ingredient.word[j] = tolower(temp_dish->ingredient.word[j]);
            }
        }
...
void input_and_judge(struct node_ty *now_dish, int *succeed, int hard, int limit_time)
{
    ...
    if (compare != 0)
        *succeed = 2; // 錯字
    else if (*succeed == 1 && gap > limit_time)
        *succeed = 3; // 超時
}
...
/*functions for maze*/
void add_to_list(struct node *tail, int x, int y)
{                                        // 新增走過哪些路
...
void pop_back(struct node *tail)
{ // 刪掉死路
...
void direction(char maze[ROWS][COLS], struct node *list, struct node *tail, int count)
{ // 創造地圖
    while (1)
    {
        ...
        if (num == 1 && maze[list->x - 2][list->y] == '0')
        { // 上
        ...
        else if (num == 2 && maze[list->x + 2][list->y] == '0')
        { // 下
            maze[list->x + 1][list->y] = '2';
            maze[list->x + 2][list->y] = '2';
            add_to_list(tail, (list->x) + 2, list->y);
        ...
        else if (num == 3 && maze[list->x][list->y - 2] == '0')
        { // 左
        ...
        else if (num == 4 && maze[list->x][list->y + 2] == '0')
        { // 右
            maze[list->x][list->y + 1] = '2';
            maze[list->x][list->y + 2] = '2';
            add_to_list(tail, list->x, (list->y) + 2);
...
void chance_card(int num, int *possible_ending, int *found_money)
{ // 機會命運可能的結果
    if (num == 1)
    {
        *found_money = 1;
...
void player(char maze[ROWS][COLS], int *x, int *y, int *possible_ending, int *found_money)
{                             // 玩家部分
...
void printf_Maze(char maze[ROWS][COLS])
{ // 印玩家走後的地圖
    for (int i = 0; i < ROWS; i++)
    {
        for (int j = 0; j < COLS; j++)
