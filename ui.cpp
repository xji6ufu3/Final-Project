...
int main(){
    ...
    /**迷宮**/
        ...
        maze[1][1] = PLAYER;
        loadimage(&img_player,"./delivery-bike.png", WALL_SIZE, WALL_SIZE);
...
{
    std::vector<node_ty*> dish(num_dish);
        ...
        for (int j = 0; j < NUM_MENU; j++) {
            if (tingyi == j) {
                ...
                if (i < num_dish - 1) {
                    ...
                    dish[i]->next = nullptr; // 最後一個 dish 的 next 設為 nullptr
                }
            }
        }
    }
...
/*functions for maze*/
void add_to_list(node* tail, int x, int y) {
    node* pre_node = tail->prior; //找到目前最後節點

    node* new_node = new node; //分配新節點
    new_node->x = x;
    new_node->y = y;
        ...
        for(int j = 0; j < COLS; j++){
            switch(maze[i][j]){
                ...
                case WALL:
                    ...
                    fillrectangle(j * WALL_SIZE, i * WALL_SIZE, j * WALL_SIZE + WALL_SIZE, i * WALL_SIZE + WALL_SIZE);
                    break;
                case PLAYER:
                putimage(j * WALL_SIZE, i * WALL_SIZE, &img_player);
                    break;
                case DOOR:
                    break;
            }
